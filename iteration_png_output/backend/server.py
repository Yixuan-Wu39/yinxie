...
from .image_adapter import ImageAdapterError, extract_text, hide_text, inspect_image
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/health":
            self._send_json({"ok": True, "version": "png-output"})
            return
        if path in {"", "/"}:
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            self._send_json({"error": "not found"}, status=404)
        except (ImageAdapterError, StegoError, ValueError, json.JSONDecodeError) as exc:
            self._send_json({"error": str(exc)}, status=400)

    def log_message(self, fmt: str, *args: object) -> None:
        print(f"[png-output] {self.address_string()} - {fmt % args}")
...
def run(host: str = "127.0.0.1", port: int = 8010) -> None:
    ...
    print(f"PNG-output image steganography server running at http://{host}:{port}")
