"""Audio adapter: WAV input or compressed audio converted to WAV, WAV output."""
...
def _to_wav(audio: bytes) -> tuple[str, bytes]:
    ...
    if _looks_like_mp3(audio):
        return "MP3", _compressed_to_wav(audio, ".mp3", "MP3")
    if _looks_like_m4a(audio):
        return "M4A", _compressed_to_wav(audio, ".m4a", "M4A")
    raise AudioAdapterError("当前只支持 WAV、MP3 和 M4A 音频")


def _compressed_to_wav(audio: bytes, suffix: str, label: str) -> bytes:
    ffmpeg = _find_ffmpeg()
    if not ffmpeg:
        raise AudioAdapterError(f"使用 {label} 功能需要先安装 ffmpeg，或先把 {label} 转成 WAV 再上传")
    ...
    try:
        ...
        raise AudioAdapterError(f"ffmpeg 解码 {label} 超时") from exc
    finally:
        if input_path:
            try:
    ...
    if proc.returncode != 0:
        ...
        raise AudioAdapterError(f"ffmpeg 解码 {label} 失败：{detail}" if detail else f"ffmpeg 解码 {label} 失败")
    try:
...
def _audio_info(source_format: str, wav_info: wav_adapter.WavInfo) -> AudioInfo:
    conversion = "无需转换" if source_format == "WAV" else f"{source_format} 已解码为 PCM WAV"
    return AudioInfo(
...
def _looks_like_m4a(audio: bytes | bytearray) -> bool:
    if len(audio) < 12:
        return False
    if bytes(audio[4:8]) != b"ftyp":
        return False
    brands = bytes(audio[8:32]).lower()
    return any(brand in brands for brand in {b"m4a ", b"m4p ", b"mp42", b"mp41", b"isom", b"iso2", b"m4b "})
